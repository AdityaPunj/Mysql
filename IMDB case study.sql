...
-- In director_mapping there are 3867 rows

SELECT Count(*) AS total_rows
FROM   genre; 
-- In genre there are 14662 rows

SELECT Count(*) AS total_rows
FROM   movie; 
-- In movie there are 7997 rows

SELECT Count(*) AS total_rows
FROM   names; 
-- In names there are 25735 rows

SELECT Count(*) AS total_rows
FROM   ratings; 
-- In ratings there are 7997 rows

SELECT Count(*) AS total_rows
FROM   role_mapping; 
-- In role_mapping there are 15615 rows
...
-- 4 columns in the movie table has null values 
...
/* By year -
The highest number of movies released is in the year 2017 and after that year the movies count decreases.
By month -
The highest number of movies released in the month 3 (March) and lowest is for month 12 (December).  */
...
WHERE  country = "usa"
        OR country = "india"
           AND year = 2019
GROUP  BY country; 
-- India has released 295 movies in year 2019
-- USA has released 2260 movies in year 2019
-- Both countries has produced 2555 movies in year 2019
...
SELECT genre,
       Count(movie_id) AS total_movies_produced_in_genre
FROM   genre g
       INNER JOIN movie m
               ON m.id = g.movie_id
GROUP  BY genre
ORDER  BY total_movies_produced_in_genre DESC
LIMIT  1;
-- Drama genre has highest number of movies produced (4285 movies)
...
SELECT Count(id) AS Number_of_movies_with_only_one_genre
FROM   genre_count; 
-- There are 3289 movies belong to 1 genre
...
FROM   genre g
       INNER JOIN movie m
               ON m.id = g.movie_id
GROUP  BY genre;
-- Thriller genre is Rank 3 in terms of number of movies produced
...
-- Dream Warrior Pictures and National Theatre Live as highest number of hit movies 
...
SELECT CASE
         WHEN (SELECT Sum(total_votes)
               FROM   country_summary
               WHERE  country LIKE "%germany%") -
              (SELECT Sum(total_votes)
               FROM   country_summary
               WHERE  country LIKE "%italy%") > 0
       THEN "yes"
         ELSE "no"
       END AS "German movies get more votes than Italian movies"; 
...
-- The Actor top on the list is Vijay Sethupathi
...
SELECT title AS "Thriller Movies",
       avg_rating,
       CASE
...
(
           ...
           ON         g.movie_id = m.id
           GROUP BY   genre
           ORDER BY   Count(g.movie_id) DESC limit 3 ), top_5_movies AS
(
           SELECT     g.genre,
                      m.year,
                      ...
                      CASE
                                 WHEN Regexp_like( worlwide_gross_income, 'INR') THEN round(CONVERT(replace( worlwide_gross_income,'INR ',''),signed)/79.01,0)
                                 ELSE CONVERT(replace( worlwide_gross_income ,'$',''),signed)
                      END   AS worlwide_gross_income_num
           FROM       movie AS m
...
FROM   (
                SELECT   genre,
                         ...
                         movie_name,
                         worlwide_gross_income,
                         dense_rank() OVER (partition BY year ORDER BY worlwide_gross_income_num DESC) AS movie_rank
                FROM     top_5_movies) AS a
WHERE  movie_rank <=5 ;
